...
def main() -> int:
    ...
    from gloss.eval.ablation import enumerate_configs, format_table, load_records, run_config

    if args.list:
        print(len(enumerate_configs(args.seeds)))
        return 0
    if args.aggregate:
        print(format_table(load_records()))
        return 0
    ...
    log.info("config[%d] = %s", args.index,
             {k: rec[k] for k in ("regime", "seed", "ap", "auroc")})
    return 0
