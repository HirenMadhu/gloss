...
class RowPool(nn.Module):
    ...
    The query is ``W_Q[u_r ; s_r]`` (the `hybrid` arm of the retired pool_query ablation) — the only
    setting any reported result used.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.w_q = nn.Linear(d_model + d_sig, self.slots * self.d_h, bias=False)
    ...
    def forward(self, h: Tensor, u: Tensor, s: Tensor, cb) -> Tensor:
        ...
        q = self.w_q(torch.cat([u, s], dim=-1)).view(B, R, M, dh)                              # [B,R,M,dh]
...
class RowAttention(nn.Module):
    ...
    **γ is name-derived** (`role_bias='name_derived'`):
    ...
    def _gamma(self, cb) -> Tensor:
        """``[B, H, R, R]`` role bias, gathered from a per-(head, role) table."""
    ...
    def forward(self, u: Tensor, s: Tensor, cb) -> tuple[Tensor, dict]:
        ...
        # |gamma| per role and per direction (§7). Near-zero magnitude means the role bias is a null
        ...
        diag = {"row_attn_entropy": _entropy(a, mask),
                ...
                "gamma_abs_per_dir": self.c_dir.detach().abs().mean(0)}
        return u + self.w_o(out), diag


class Broadcast(nn.Module):
    """§3.6 — high→low. ``h_i += W_b u_{ν(i)}``.

    Every cell of a row receives the SAME vector; a cell cannot pick which FK-adjacent row to read.
    That limitation is deliberate and measured, not an oversight: the attention variant that lifts it
    (``archive/multi-level/model/row_to_cell.py``) was worse on 7 of 8 tasks.

    Note it reads ``u`` unnormalised — the only cross-level operator besides :class:`RowPool` without
    a pre-norm. Untested either way; noted so it is not mistaken for a considered choice.
    """
