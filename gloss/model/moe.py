...
hidden state ``x``. Dense expert combine — every expert runs on every token — is the simple, correct
MVP (true sparse dispatch is deferred). Balance is by a router-weight **orthogonality** loss, not a
uniform load-balancing aux loss, so usage may follow the long tail of relation frequencies.
"""
...
class MoEFFN(nn.Module):
    ...
    The S/C/P/H ablation additions (shared expert, cosine router, top-p support, hierarchical gate)
    are retired to ``archive/multi-level/model/moe_ablations.py``. Note the ROW-level MoE keeps its
    own shared expert (``RowMoE(use_shared=True)``); that asymmetry is deliberate and measured.

    The combine is DENSE — every expert runs on every token — so this costs ``num_experts x`` the
    FFN FLOPs of a plain SwiGLU. True sparse dispatch is deferred, so do not claim active-FLOP parity.
    """
