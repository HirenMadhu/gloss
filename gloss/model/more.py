...
ablation arms, the hidden / value / identity). ``forward`` returns ``(logits, aux)`` where ``aux`` is the
summed router-orthogonality loss (0 for the dense arms). ``route_on``:

    signature : route on z (the method)                          identity : learned per-column id (no transfer)
    hidden    : route on the block's evolving hidden state       dense    : plain RT (no MoE)
    value     : route on the cell's value component              dense_wide: param-matched dense (d_ff x k)
"""
...
ROUTE_ONS = ("signature", "hidden", "value", "identity", "dense", "dense_wide")
